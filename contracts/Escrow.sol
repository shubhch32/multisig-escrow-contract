...
contract Escrow is Ownable, ERC721 {
    ...
    event Deposit(address indexed recipient, uint indexed claimTokenId);
     ...
     *
     */
    function getClaimDetails(address account) public view returns (uint256, uint256) {
        return (
        escrowBalances[account].assets.length,
    ...
    function deposit(Asset[] memory assets) payable external {
        for (uint i = 0; i < assets.length; i++){
            ...
            require(asset.amount != 0 , "need to supply amount");
            ...
            emit Deposit(asset.recipient, escrowBalances[asset.recipient].claimTokenID);
        }
    ...
    function withdraw() external {
        require(escrowBalances[msg.sender].erc20 > 0, "nothing to withdraw");
