...
contract MultiSig {
    ...
    function executeTransaction(uint transactionId) public {
        require(isConfirmed(transactionId),"Transaction has not been confirmed by sufficient number of signers");
        emit Execution(transactionId);
        Transaction storage _tx = transactions[transactionId];
//        (bool success, ) = _tx.destination.call{ value: _tx.value }(_tx.data);
        Token instance = Token(tokenContractAddress);//@@
        bool success = instance.transfer(_tx.destination, _tx.value);//@@
    ...
    constructor(address[] memory _owners, uint _confirmations, address _tokenContractAddress) {//@@
